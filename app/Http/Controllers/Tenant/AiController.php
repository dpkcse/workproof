...
class AiController extends Controller
{
    ...
    public function taskCleaner(Request $request, CurrentWorkspace $cw): JsonResponse
    {
        ...
        $response = $this->ai->generate($cw->required(),'task_cleaner','Clean this raw task text. Return JSON with cleaned_title, cleaned_description, suggested_priority, suggested_checklist_items. Suggestion only; do not decide automatically.', ['raw_text'=>$data['raw_text']]);
    ...
    public function checklistGenerator(Request $request, CurrentWorkspace $cw): JsonResponse
    {
        ...
        $response = $this->ai->generate($cw->required(),'checklist_generator','Generate practical checklist items for this task as JSON array. Do not add them automatically. Cite task ID if available.', $context, ['source_type'=>'task','source_ids'=>array_filter([$task?->id])]);
    ...
    {
        $w=$cw->required();$tasks=Task::query()->where('workspace_id',$w->id)->where(fn($q)=>$q->where('due_date','<',now()->toDateString())->whereNot('status','completed')->orWhere('priority','high')->orWhere('status','reopened'))->limit(50)->get(['id','title','priority','status','due_date']);$missing=MissingDailyReport::query()->where('workspace_id',$w->id)->where('status','open')->limit(50)->get(['id','user_id','report_date']);
    ...
    {
        $w=$cw->required();$tasks=Task::query()->where('workspace_id',$w->id)->where(fn($q)=>$q->whereIn('status',['in_progress','reopened','hold'])->orWhere('due_date','<',now()->toDateString()))->limit(50)->get(['id','title','status','priority','due_date']);
    ...
    {
        $data=$request->validate(['week_start'=>['required','date']]);$w=$cw->required();$start=\Carbon\Carbon::parse($data['week_start'])->startOfWeek();$end=$start->copy()->endOfWeek();
